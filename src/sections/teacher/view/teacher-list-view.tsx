...
const TABLE_HEAD: TableHeadCellProps[] = [
  { id: 'name', label: 'الاسم' },
  { id: 'currentCourses', label: 'الدورة الحالية', width: 150 },
  { id: 'totalCourses', label: 'عدد الدورة', width: 150 },
  { id: '', width: 88 },
];
...
export function TeachersListView() {
  ...
  const [tableData, setTableData] = useState<ITeacherItem[]>([
    {
      _id: '1',
      name: 'سيد محمد',
      pic: '',
  ...
  const renderConfirmDialog = () => (
    <ConfirmDialog
      ...
      content={
        <>
          هل تريد حذف <strong> {table.selected.length} </strong> المعلم؟
        </>
      }
      action={
        <Button
  ...
  return (
    <>
      <DashboardContent>
        <CustomBreadcrumbs
          heading="المدرسين"
          links={[{ name: 'لوحة التحكم', href: paths.dashboard.root }, { name: 'المدرسين' }]}
          action={
            <Button
              variant="contained"
      ...
      <TeacherQuickEditForm open={isNew.value} onClose={isNew.onFalse} />
      {renderConfirmDialog()}
    </>
  );
}
